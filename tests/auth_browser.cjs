...
(async () => {
 ...
 try {
  ...
  await page.fill('#city', 'Kolkata'); await page.fill('#country', 'India');
  ...
  await page.goto('/auth?mode=signup');await page.fill('#name','Duplicate');await page.fill('#initials','ST');await page.fill('#password','long-password-123');await page.fill('#security-answer','other');await page.fill('#city', 'Kolkata'); await page.fill('#country', 'India');await page.click('#submit');await page.waitForFunction(()=>document.getElementById('message').textContent.includes('already have an account'));
